...
export function ChatWidget() {
  ...
  return <div className="chat-widget">
    {open && expanded && <button className="chat-backdrop" aria-label="ย่อหน้าต่างแชต" onClick={() => setExpanded(false)} />}
    {open && <div className={`chat-panel ${expanded ? "expanded" : ""}`} role="dialog" aria-label="แชตกับ ARCANA"><div className="chat-panel-head"><span><i>✦</i> ARCANA Support<small>พร้อมช่วยคุณเลือกสิ่งที่ใช่</small></span><div><button className="chat-expand" onClick={() => setExpanded((value) => !value)} aria-label={expanded ? "ย่อหน้าต่างแชต" : "ขยายหน้าต่างแชต"}>{expanded ? "↙" : "↗"}</button><button onClick={() => { setOpen(false); setExpanded(false); }} aria-label="ปิดแชต">×</button></div></div>
    ...
    <button className="chat-button" onClick={() => { setOpen((value) => !value); if (open) setExpanded(false); }} aria-expanded={open} aria-label="เปิดแชต"><Icon name="chat" /><span>แชตกับเรา</span></button>
  </div>;
}
